...
module.exports = function defineGrammar(dialect) {
  return grammar(JavaScript, {
    ...
    supertypes: ($, previous) => previous.concat([$.type, $.primary_type]),
    ...
    rules: {
      ...
      as_expression: ($) =>
        prec.left("binary", seq($.expression, "as", choice("const", $.type))),

      satisfies_expression: ($) =>
        prec.left("binary", seq($.expression, "satisfies", $.type)),
      ...
      template_type: ($) =>
        seq("${", choice($.primary_type, $.infer_type), "}"),
      ...
      infer_type: ($) =>
        prec.right(
          seq("infer", $._type_identifier, optional(seq("extends", $.type))),
        ),
