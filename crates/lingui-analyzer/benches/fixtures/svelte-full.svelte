...
<script lang="ts">
  ...
  const ownerGreeting = $derived($translate`Welcome back, ${dashboardOwner}.`);
  ...
  const statusBanner = $derived(
    alertLevel === "error"
      ? $translate`Escalation required: unresolved high-priority queue items remain.`
      : alertLevel === "warn"
        ? $translate`Attention needed: high-priority items are waiting for review.`
        : $translate`All monitored queues are stable.`,
  );
  ...
  >
    <div class="flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="grid min-w-64 gap-2 rounded-2xl bg-slate-50 p-4">
        ...
        <p class="text-xs text-slate-500">
          {$translate`Workspace mode:`}
          {workflowLabel}
        </p>
        <p class="text-xs text-slate-500">
          {workspaceWritable
            ? $translate`Editing tools are enabled for this workspace.`
            : $translate`Editing tools are read-only for this workspace.`}
        </p>
      </div>
    </div>
    ...
    <div class="flex flex-wrap gap-3 text-sm">
      <span class="rounded-full border px-3 py-2"
        >{$translate`Region:`} {selectedRegion}</span
      >
      <span class="rounded-full border px-3 py-2"
        >{$translate`Queue kind:`} {selectedKind}</span
      >
      <span class="rounded-full border px-3 py-2">
        {$translate`Assigned only:`}
        {showAssignedOnly ? $translate`on` : $translate`off`}
      </span>
      <span class="rounded-full border px-3 py-2">
        {$translate`History:`}
        {showHistory ? $translate`shown` : $translate`hidden`}
      </span>
    </div>
  </header>
        ...
        {#each $pagedQueue as item (item.id)}
          ...
          <article class="grid gap-3 rounded-2xl border border-slate-200 p-4">
            ...
            <div class="grid gap-2 text-sm text-slate-700 md:grid-cols-2">
              <p>{$translate`Owner:`} {item.owner}</p>
              <p>{$translate`Assignee:`} {item.assignee}</p>
              <p>{$translate`Comments:`} {item.comments}</p>
              <p>{$translate`Attachments:`} {item.attachments}</p>
            </div>
            ...
            <div class="flex flex-wrap gap-3 text-sm text-slate-600">
              ...
              {#if item.pathHint}
                <span class="rounded-full bg-slate-100 px-3 py-1">
                  {$translate(msg`Path ${String(item.pathHint ?? "")}`)}
                </span>
              {:else}
                <span class="rounded-full bg-slate-100 px-3 py-1">
                  {$translate`No path`}
      ...
      >
        ...
        {#if $highlightedQueue.length === 0}
          <p class="rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">
            {$translate`No highlighted queue items.`}
          </p>
        {:else}
          <ul class="grid gap-3">
