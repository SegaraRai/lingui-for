...
use crate::framework::parse::{ParseError, parse_astro, parse_svelte, parse_tsx};
...
pub(crate) fn lower_runtime_component_markup_astro(
    source_name: &str,
    original_source: &str,
    target: &CompileTarget,
    declaration: &RenderedMappedText,
    ...
    convert_runtime_trans_root_astro(
        &original_source,
        declaration.indexed_source_map.as_ref(),
        &declaration_source,
        &mapped_input,
...
fn convert_runtime_trans_root_astro(
    original_source: &IndexedText<'_>,
    declaration_source_map: Option<&IndexedSourceMap>,
    source: &IndexedText<'_>,
    input: &MappedText<'_>,
        ...
        match child.kind() {
            "jsx_expression" => {
                ...
                if let Some(object) = lowerable_object_expression_node(argument) {
                    let spread_span = translated_span(spread, base_offset)?;
                    let object_span = translated_span(object, base_offset)?;
                    let lowered = lower_astro_object_expression_span(
                        source.as_str(),
                        input,
                        &original_input,
                        original_source.as_str(),
            ...
            "jsx_attribute" => {
                ...
                if name == "components" {
                    ...
                    {
                        if let Some(expression) = first_named_child(value) {
                            if let Some(component_slots) = collect_astro_component_slot_callbacks(
                                &original_input,
                                original_source.as_str(),
                                target,
                                source.as_str(),
...
fn lower_astro_object_expression_span(
    source: &str,
    input: &MappedText<'_>,
    original_input: &MappedText<'_>,
    original_source: &str,
    ...
    convert_astro_object_expression(
        source,
        input,
        original_input,
        original_source,
...
fn convert_astro_object_expression(
    source: &str,
    input: &MappedText<'_>,
    original_input: &MappedText<'_>,
    original_source: &str,
...
fn collect_astro_component_slot_callbacks(
    original_input: &MappedText<'_>,
    original_source: &str,
    target: &CompileTarget,
    transformed_source: &str,
    ...
    Ok(Some(collect_astro_component_slot_callbacks_from_source(
        original_input,
        original_source,
        target,
        &keys,
...
fn collect_astro_component_slot_callbacks_from_source(
    original_input: &MappedText<'_>,
    original_source: &str,
    target: &CompileTarget,
    keys: &[String],
    ...
    let mut wrappers = Vec::new();
    collect_astro_runtime_component_wrappers(component_node, original_source, &mut wrappers);
    ...
    keys.iter()
        .zip(wrappers)
        .map(|(key, wrapper)| {
            lower_original_astro_wrapper_to_slot_callback(
                original_input,
                wrapper,
                &format!("component_{key}"),
            )
...
fn collect_astro_runtime_component_wrappers<'a>(
    node: Node<'a>,
    source: &str,
    wrappers: &mut Vec<Node<'a>>,
) {
        ...
        match child.kind() {
            "element" => {
                if let Some(self_closing_tag) = child
                    ...
                    .find(|grandchild| grandchild.kind() == "self_closing_tag")
                {
                    if !is_skipped_astro_runtime_component_wrapper(source, self_closing_tag) {
                        wrappers.push(self_closing_tag);
                    }
                    continue;
                }

                if !is_skipped_astro_runtime_component_wrapper(source, child) {
                    wrappers.push(child);
                }
                collect_astro_runtime_component_wrappers(child, source, wrappers);
            }
            "self_closing_tag" => {
                if !is_skipped_astro_runtime_component_wrapper(source, child) {
                    wrappers.push(child);
            ...
            _ => collect_astro_runtime_component_wrappers(child, source, wrappers),
        }
    }
}

fn is_skipped_astro_runtime_component_wrapper(source: &str, node: Node<'_>) -> bool {
    matches!(
        astro_tag_name(source, node),
        Some("Plural" | "Select" | "SelectOrdinal" | "Trans")
    )
}

fn astro_tag_name<'a>(source: &'a str, node: Node<'_>) -> Option<&'a str> {
    match node.kind() {
        "element" => node
            .children(&mut node.walk())
            .find(|child| child.kind() == "start_tag")
...
fn lower_original_astro_wrapper_to_slot_callback(
    input: &MappedText<'_>,
    node: Node<'_>,
    slot_name: &str,
) -> Result<RenderedMappedText, RuntimeComponentError> {
...
mod tests {
    use super::{
        lower_runtime_component_markup, lower_runtime_component_markup_astro,
        lower_runtime_component_markup_svelte,
    };
    ...
    fn astro_component_target(source: &str) -> CompileTarget {
        CompileTarget {
            declaration_id: "__trans".to_string(),
            original_span: Span::new(0, source.len()),
            normalized_span: Span::new(0, source.len()),
    ...
    #[test]
    fn lowers_astro_components_to_named_slot_callbacks() {
        let source = "<Trans>Read the <a href=\"/docs\">docs</a>.</Trans>".to_string();
        let declaration = RenderedMappedText {
            code: "<Trans {.../*i18n*/ { id: \"demo.docs\", message: \"Read the <0>docs</0>.\", components: { 0: <a href=\"/docs\" /> } }} />".to_string(),
            indexed_source_map: None,
        };
        let target = astro_component_target(&source);

        let lowered = lower_runtime_component_markup_astro(
            "Component.astro",
            &source,
            &target,
            &declaration,
    ...
    #[test]
    fn lowers_astro_components_from_original_source_wrappers() {
        let source =
            "<Trans>Read <strong><DocLink href=\"/docs\">carefully</DocLink></strong>.</Trans>"
                .to_string();
        let declaration = RenderedMappedText {
