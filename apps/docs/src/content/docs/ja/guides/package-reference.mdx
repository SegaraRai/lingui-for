...
| `lingui-for-svelte/macro`      | `t`, `plural`, `select`, `selectOrdinal`, `Trans`, `Plural`, `Select`, `SelectOrdinal`, `msg`, `defineMessage`, `ph` | ✓          |
...
| `lingui-for-astro/macro`       | `t`, `plural`, `select`, `selectOrdinal`, `Trans`, `Plural`, `Select`, `SelectOrdinal`, `msg`, `defineMessage`, `ph` | ✓          |
...
`whitespace`、`packages`、`runtimeWarnings` などのフレームワーク固有のオプションが必要な場合は、`lingui-for-svelte/config` または `lingui-for-astro/config` の `defineConfig` を使います。
フレームワークのビルド変換とエクストラクタは同じ設定を読み込み、対応する `framework.svelte` または `framework.astro` セクションを参照します。
単一フレームワークのプロジェクトと複数フレームワークのプロジェクトでの動きについては、[フレームワーク設定](/ja/guides/framework-config) を参照してください。
