...
When a framework transform has configurable behavior, put those settings under the matching
`framework` key in `lingui.config.ts`. The build transform and extractor both load the same config,
so a setting such as rich-text Component Macro whitespace can live in one place:
...
See [Whitespace in Component Macros](/guides/whitespace-in-component-macros) for details.

Lingui directives are another feature that must agree across these two paths. The framework
extractor and build transform interpret the same directive state, including directives that cross
an Astro frontmatter or Svelte script boundary. When adding a new comment form, verify extraction
and transform independently. See [Lingui Directives](/guides/lingui-directives) for the supported
