...
describe.sequential("dev hydration", () => {
  ...
  test(
    "switches locale from the header during dev navigation",
    async () => {
      const switched = await commands.switchLocaleFromHeader(
        "/?lang=en",
        "ja",
      );
  ...
  test(
    "updates the html lang attribute during dev navigation",
    async () => {
      const switched = await commands.switchLocaleFromHeader(
        "/?lang=en",
        "ja",
  ...
  test("renders the component playground route without hydration errors", async () => {
    ...
    expect(components.bodyText).toContain("コンポーネントタスクは 2 件待機中です");
  ...
  test("renders explicit-id playground route without hydration errors", async () => {
    const result = await commands.captureHydrationErrors("/playground/ids?lang=ja");

    expect(result.bodyText).toContain("id、コメント、コンテキストの確認に絞る");
    expect(result.bodyText).toContain("Trans コンポーネントからの明示的な id。");
    expect(result.errors).toEqual([]);
  });
});
